...
An output type that streams video and audio from Smelter over HLS. Register it before use; see `routes.md` for output management.
...
### max_playlist_size
Number of segments kept in the playlist. When the limit is reached the oldest segment is removed. If not specified, no segments will be removed.
