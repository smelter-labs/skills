...
An output type that provides a WHEP server endpoint, allowing a media stream to be broadcast to multiple clients simultaneously.

Smelter exposes the WHEP endpoint on port 9000 under the `/whep/:output_id` route. A different port can be configured with the `SMELTER_WHIP_WHEP_SERVER_PORT` environment variable. To connect a new output, register it first (see `routes.md`), then establish the WHEP connection.

> **Note:** This output complies with the WHEP specification described in draft-ietf-wish-whep-02.
...
### video
Parameters of the video included in the WHEP stream.
...
### audio
Parameters of the audio included in the WHEP stream.
