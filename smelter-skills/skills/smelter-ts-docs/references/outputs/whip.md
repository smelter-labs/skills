...
Each entry of `video.encoderPreferences` is one of the following (plus `{ type: "any" }`, which means "any supported negotiated codec"):
...
Each entry of `audio.encoderPreferences` is `{ type: "opus" }` (below) or `{ type: "any" }` (any supported negotiated codec).
