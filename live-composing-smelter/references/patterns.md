...
- [Zustand Store Bridge](#zustand-store-bridge)
...
Smelter renders React components server-side (not DOM). Use a Zustand store to bridge external state (API, events) into the React rendering tree.
...
type RoomStore = {
  ...
  updateState: (inputs: InputConfig[], layout: string) => void;
};
...
function createRoomStore(): StoreApi<RoomStore> {
  return createStore<RoomStore>(set => ({
    ...
    updateState: (inputs, layout) => set({ inputs, layout }),
  }));
}

const StoreContext = createContext<StoreApi<RoomStore>>(createRoomStore());

// Root component — pass store via context
function App({ store }: { store: StoreApi<RoomStore> }) {
  return (
...
function OutputScene() {
  ...
  // ...render based on layout and inputs
}
```

**Key points:**
- Create one store per output (room)
- External code (API handlers, event listeners) calls `store.getState().updateState(...)` to trigger re-renders
- React components consume store via `useContext` + `useStore`
